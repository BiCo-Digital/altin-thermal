...
while(cap.isOpened()):
    ...
    skeleton[skeletonize(mask_hot)] = 255
    display('skeleton', skeleton, (3, 1))

    # detect lines in skeleton
    lines = cv2.HoughLinesP(gray, 1, np.pi/180, 100, minLineLength=10, maxLineGap=10)
    if lines is not None:
        print(lines)
        for line in lines:
            x1,y1,x2,y2 = line[0]
            cv2.line(frame,(x1,y1),(x2,y2),(0,255,0),2)
    display('Lines', frame, (3, 0))


    # detect contours
    contours_cold, _ = cv2.findContours(mask_cold, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours_hot, _ = cv2.findContours(mask_hot, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
