...
DIR_NAME = "/Users/matejnevlud/github/altin-thermal/thermal_images_2023-12-11_21-51-38"
thermal_images = []
...
for filename in filenames:
    #read images png in f 32 with 1 channel
    frame = cv2.imread(os.path.join(DIR_NAME, filename), cv2.IMREAD_UNCHANGED).astype(np.float32)
    thermal_images.append(frame)
...
# make ax 1/3 of the width, and ax2 2/3 of the width
ax = fig.add_axes([0.0, 0.0, 0.33, 1.0])
ax2 = fig.add_axes([0.33, 0.0, 0.67, 1.0], projection='3d')
...
while 1:
    ...
    frame = soups_frame
    frame = np.clip(frame, min_t, 30)

    X = np.arange(0, min(frame.shape[1], frame.shape[0]), 1)
    Y = np.arange(0, min(frame.shape[1], frame.shape[0]), 1)
    X, Y = np.meshgrid(X, Y)

    Z = frame[:min(frame.shape[1], frame.shape[0]), :min(frame.shape[1], frame.shape[0])]

    ax2.cla()
    ax2.plot_surface(X, Y, Z, cmap='jet', antialiased=False)

    #for hot_line in hot_lines:
        #ax2.plot([hot_line[0][0], hot_line[1][0]], [hot_line[0][1], hot_line[1][1]], zs=35, color='r', linewidth=3)
